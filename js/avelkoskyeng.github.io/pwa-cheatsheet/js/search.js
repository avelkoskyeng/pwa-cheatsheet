...
function renderItemsList(items, options = {}) {
  ...
  const iconPath = (window.BASE_PATH || '') + '/icons/arw-rght.svg';
      ...
      ${items
        .map((item) => {
          ...
          if (href.includes('.html')) {
            ...
            // Если путь содержит /pages/ и нужно добавить BASE_PATH
            if (basePath && href.includes('/pages/')) {
              href = basePath + href;
            }
            // Если это относительный путь pages/... и есть BASE_PATH
          ...
          <a class="search-results__item" href="${href}">
            ...
            <img src="${iconPath}" class="search-results__arrow" alt="" aria-hidden="true" />
          </a>
        `;
        })
        .join('')}
